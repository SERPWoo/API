...
//
// This code requests a project's Tagged domains/urls and outputs the Domain ID, Domain/URL, ORM Tag, Settings, Creation Date, Last Updated
//
// This outputs to the Google Sheet
//
// Last updated - May 5th, 2019 @ 11:05 EST (@MercenaryCarter https://github.com/MercenaryCarter and https://twitter.com/MercenaryCarter)
//
//	Instructions:
...
function PullTaggedDomains() //Pulls the API Data
{
	...
	var domain_array		= new Array();
		...
		for (var project_id in json_response.projects) {
				...
				if( typeof (json_response.projects[project_id]) === "object" ) {
					...
					for (var domain_id in json_response.projects[project_id].tags) {
					...
					for (var j = 0 ; j < domain_array.length ; ++j) {
						...
						sheet.getRange((j+2), 2).setValue(json_response.projects[project_id].tags[the_domain_id].tag);
						sheet.getRange((j+2), 3).setValue(json_response.projects[project_id].tags[the_domain_id].orm);

							//Whether this is an exact URL or just monitoring the whole domain
							if ((json_response.projects[project_id].tags[the_domain_id].setting.type !== undefined) && (json_response.projects[project_id].tags[the_domain_id].setting.type !== null)) {
								sheet.getRange((j+2), 4).setValue(json_response.projects[project_id].tags[the_domain_id].setting.type);  							
							}else {
								sheet.getRange((j+2), 4).setValue('domain');
							}

						sheet.getRange((j+2), 5).setValue(FROM_UNIX_EPOCH(json_response.projects[project_id].tags[the_domain_id].creation_date));

							if ((json_response.projects[project_id].tags[the_domain_id].last_updated !== undefined) && (json_response.projects[project_id].tags[the_domain_id].last_updated !== null)) {
								sheet.getRange((j+2), 6).setValue(FROM_UNIX_EPOCH(json_response.projects[project_id].tags[the_domain_id].last_updated));
							}else {
								sheet.getRange((j+2), 6).setValue(FROM_UNIX_EPOCH(json_response.projects[project_id].tags[the_domain_id].creation_date));
							}
...
function onOpen() {
	SpreadsheetApp.getUi().createMenu("[ SERPWoo Script ]").addItem("Pull Tagged Domains", "PullTaggedDomains").addToUi(); 
}
