...
//
//	Last updated - Aug 27th, 2017 @ 22:04 EST (@MercenaryCarter https://github.com/MercenaryCarter and https://twitter.com/MercenaryCarter)
//
//	You might need to install curl: apt-get install libcurl4-openssl-dev
//
//	Compile command: gcc -ansi -pedantic -Wall -Werror -std=gnu99 -o list-all-project-notes list-all-project-notes.c -lm -lcurl
//	Run Command: ./list-all-project-notes
//
...
int main(int argc, char** argv)
{
		...
		the_statement = snprintf(final_request_URL, 255, "%s/projects/%d/notes/?key=%s", request_url, Project_ID, API_key);
...
static void List_Row(){
    ...
    printf("%-25s %-15d %-10d %-80s\n",currentId, currentTimestamp, currentType, currentMessage);
    selectedTotal = 0;
...
static void process_object(json_value* value, int depth)
{
        ...
        for (x = 0; x < length; x++) {
		    ...
		    //printf("%-15d depth=[%d] // %s\n", selectedTotal, depth, value->u.object.values[x].name);
				...
				if(depth == 11 && strcmp(value->u.object.values[x].name, "message") == 0 ) {
                            currentMessage = value->u.object.values[x].value->u.string.ptr;
                            selectedTotal++;
						    //printf("currentMessage %-15d %-15s\n", selectedTotal, currentMessage);
                }
				
				if(depth == 11 && strcmp(value->u.object.values[x].name, "timestamp") == 0 ) {
                            currentTimestamp = value->u.object.values[x].value->u.integer;
                            selectedTotal++;
						    //printf("currentTimestamp %-15d %-15d\n", selectedTotal, currentTimestamp);
                }				

				if(depth == 9 && strcmp(value->u.object.values[x].name, "type") == 0 ) {
                            currentType = value->u.object.values[x].value->u.integer;
                            selectedTotal++;
						    //printf("currentType %-15d %-15d\n", selectedTotal, currentType);
                }						

                if(selectedTotal == 4) { //corresponds with total amount of variables we are looking for
                    List_Row();
                }
